...
export default function PerangkatPage() {
    ...
    // QR Code
    const openQrModal = (device: Device) => {
        setSelectedDevice(device);
        setShowQrModal(true);
    };

    // Scan Ulang (reconnect)
    const handleScanUlang = (device: Device) => {
        setDevices(devices.map(d =>
            d.id === device.id
                ? { ...d, status: "connected" as const, battery: "92%", batteryIcon: "battery_5_bar", lastSync: "Sinkronisasi: Baru saja" }
                : d
        ));
        showToast(`Perangkat "${device.name}" berhasil terhubung kembali`);
    };
                    ...
                    >
                        ...
                        <div className="bg-gray-50 px-6 py-3 border-t border-gray-100 flex justify-end gap-2">
                            ...
                            {device.status === "connected" ? (
                                ...
                                <button
                                    onClick={() => handleScanUlang(device)}
                                    className="bg-wa-green hover:bg-wa-dark text-white px-3 py-1.5 rounded-lg text-xs font-medium transition-colors shadow-sm"
                                >
                                    Scan Ulang
                                </button>
                            )}
            ...
            {showQrModal && selectedDevice && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowQrModal(false)}>
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                            <h3 className="text-lg font-bold text-gray-900">QR Code Perangkat</h3>
                        ...
                        <div className="p-6 flex flex-col items-center gap-4">
                            ...
                            <div className="text-center">
                                <h4 className="font-bold text-gray-900">{selectedDevice.name}</h4>
                                <p className="text-sm text-gray-500">{selectedDevice.phone}</p>
                            </div>
                            <p className="text-xs text-gray-400 text-center">Scan QR code ini dengan WhatsApp di perangkat Anda untuk menghubungkan ulang sesi.</p>
                        </div>
                        <div className="flex justify-end px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-xl">
                            <button onClick={() => setShowQrModal(false)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
                                Tutup
                            </button>
                        </div>
                    </div>
